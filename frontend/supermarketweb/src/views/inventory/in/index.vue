...
const formRules = {
  ioNo: [{ required: true, message: '请输入入库单号', trigger: 'blur' }],
...
const handleDelete = (index, row) => {
  ElMessageBox.confirm(`确定要删除入库单"${row.ioNo}"吗？`, '警告', {
...
const handleSubmit = () => {
  ElMessage.success(dialogTitle.value === '新增入库单' ? '添加成功' : '更新成功')
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="入库单号" prop="ioNo">
              <el-input v-model="formData.ioNo" placeholder="请输入入库单号" clearable :disabled="!!formData.id" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
